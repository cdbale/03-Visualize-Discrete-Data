...
Imagine our client, Patagonia, has been happy with the results we've obtained from transforming the data (filtering, mutating, joining, slicing, etc.). Suppose that your results have been shared with the technical team, but that stakeholders are asking for a presentation on your results. What are some concise and effective ways that you could communicate data-based insights about Patagonia's customer base?
...
The `position` argument of the `geom_col()` function is set to `stack` by default. Try `fill` instead.
...
Free-form text is **unstructured**. Tokenization turns it into word categories that we can count and visualize.
